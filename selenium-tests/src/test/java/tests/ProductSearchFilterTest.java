...
public class ProductSearchFilterTest extends BaseTest {
    ...
    @Test
    public void validateAdminCanFindProductThroughNameAfterLogin() {
        AdminDashboardPage adminDashboardPage = LoginUtils.loginAsAdmin(driver, BASE_URL)
                .waitForProductListingLoaded();
        String productName = adminDashboardPage.getFirstVisibleProductName();

        adminDashboardPage.searchByName(productName);

        Assert.assertTrue(adminDashboardPage.getVisibleProductCount() > 0, "Search should return at least one admin product.");
        Assert.assertTrue(adminDashboardPage.hasProductNamed(productName), "Search results should include the searched product name.");
        Assert.assertTrue(adminDashboardPage.allVisibleProductsMatchSearchTerm(productName), "Every visible admin result should match the search term.");
    }
    ...
    @Test
    public void validateAdminCanFilterProductsThroughCategoryAfterLogin() {
        AdminDashboardPage adminDashboardPage = LoginUtils.loginAsAdmin(driver, BASE_URL)
                .waitForProductListingLoaded();
        String category = requireFilterableCategory(adminDashboardPage.getFirstFilterableCategory());

        adminDashboardPage.filterByCategory(category);

        Assert.assertTrue(adminDashboardPage.getVisibleProductCount() > 0, "Category filter should return at least one admin product.");
        Assert.assertTrue(adminDashboardPage.allVisibleProductsHaveCategory(category), "Every visible admin result should match the selected category.");
    }
